...
def build_sentiment_prompt(title, text):
    """Create the revised three-class prompt for one review."""
...
Choose POSITIVE only when the review has a clearly positive overall feeling.
The reviewer should be satisfied, enthusiastic, recommend the product, or
describe a clearly good experience without meaningful negative comments.

Choose NEGATIVE only when the review has a clearly negative overall feeling.
The reviewer should be dissatisfied, describe a clear problem, or describe a
bad experience without meaningful positive comments.

Choose NEUTRAL whenever the review has mixed feelings, mixed positive and
negative statements, limited enthusiasm, or uncertainty.

Use these specific NEUTRAL rules:

1. If the review text is only the one word "good", classify it as NEUTRAL.
   Do this even if the title is positive.

2. If the title is positive or negative but the text includes both positive and
   negative feelings, classify it as NEUTRAL.

3. If the title and text conflict, or the combined title and text communicate
   mixed feelings, classify it as NEUTRAL.

4. If the review says the item was "fine", "not bad", satisfactory, or
   acceptable while also mentioning a problem, disappointment, dissatisfaction,
   or another negative statement, classify it as NEUTRAL.

5. If the review is somewhat positive or somewhat negative but lacks clear
   enthusiasm or strong dissatisfaction, classify it as NEUTRAL.

Only use POSITIVE or NEGATIVE when the overall feeling is clear and consistent.
...
def classify_review(title, text):
    ...
    with request.urlopen(model_request, timeout=60) as response:
